...
import type { Session, SessionRetailItem, Client, ClientServiceFormula, MaterialEntry } from '@/types/models';
...
export default function SessionsPage() {
  ...
  // Edit-materials modal handlers (named so they can be passed to MaterialsList)
  const handleEditMaterialProductSelect = (index: number, productId: string) => {
    const product = products?.find((p) => p.id === productId);
    if (!product) return;
    const updated = [...editMaterials];
    const isServiceCost = product.type === 'service_cost';
    const oldMat = editMaterials[index];
    const availStock = isServiceCost
      ? Number.MAX_SAFE_INTEGER
      : product.currentStock + (oldMat.productId === productId ? oldMat.quantity : 0);
    const qty = Math.min(updated[index].quantity, availStock);
    updated[index] = {
      ...updated[index],
      productId,
      productName: product.name,
  ...
  const handleEditMaterialQuantityChange = (index: number, qty: number) => {
    const updated = [...editMaterials];
    const capped = Math.min(Math.max(0, qty), updated[index].maxStock || Infinity);
    updated[index] = { ...updated[index], quantity: capped, totalPrice: updated[index].pricePerUnit * capped };
    setEditMaterials(updated);
  };
  const handleEditMarkUsage = (index: number) => {
    const updated = [...editMaterials];
    const cur = updated[index];
    const next = Math.min(cur.quantity + cur.defaultUsage, cur.maxStock || Infinity);
    updated[index] = { ...cur, quantity: next, totalPrice: cur.pricePerUnit * next };
  ...
  const handleEditResetUsage = (index: number) => {
    const updated = [...editMaterials];
    updated[index] = { ...updated[index], quantity: 0, totalPrice: 0 };
    setEditMaterials(updated);
  };
  const toggleEditManualOverride = (index: number, manual: boolean) => {
    const updated = [...editMaterials];
    updated[index] = { ...updated[index], manualOverride: manual };
    setEditMaterials(updated);
  };
  const handleEditAddRow = () => {
    setEditMaterials([...editMaterials, { productId: '', productName: '', quantity: 1, unit: '', pricePerUnit: 0, totalPrice: 0, maxStock: 0, imprecise: false, defaultUsage: 0, manualOverride: false }]);
  };
          ...
          {serviceForm.serviceId && (
            <div>
              ...
              <MaterialsList
                ...
                onSelectProduct={handleMaterialProductSelect}
                onChangeQuantity={handleMaterialQuantityChange}
                onRemove={handleRemoveMaterial}
                onMarkUsage={handleMarkUsage}
                onResetUsage={handleResetUsage}
                onToggleManualOverride={toggleManualOverride}
                onAddRow={handleAddMaterialRow}
                prefillNotice={hasPrefilledMaterials ? 'Cargado de la última vez con esta clienta · Revisá y ajustá si es necesario' : undefined}
              />
            </div>
          )}
      ...
      >
        <div className="space-y-4 pb-16 sm:pb-0">
          <MaterialsList
            ...
            productOptions={productOptions}
            recentProductIds={recentProductIds}
            onSelectProduct={handleEditMaterialProductSelect}
            onChangeQuantity={handleEditMaterialQuantityChange}
            onRemove={(idx) => setEditMaterials(editMaterials.filter((_, i) => i !== idx))}
            onMarkUsage={handleEditMarkUsage}
            onResetUsage={handleEditResetUsage}
            onToggleManualOverride={toggleEditManualOverride}
            onAddRow={handleEditAddRow}
          />
          <div className="flex gap-2 pt-2">
