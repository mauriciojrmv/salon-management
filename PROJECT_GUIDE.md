...
#### P11-HIGH — Flaky-Wifi Resilience (Offline-First Foundation)

Salon runs on its own wifi but the signal drops intermittently. Without mitigation, taps on Tomar / start / pause hang silently, workers double-tap, and two workers can take the same queue entry when `onSnapshot` is behind. This subsection covers the foundation: persistent local cache, atomic take, and an honest connection indicator.

- [x] **Firestore persistent local cache** — Fixed 2026-04-21: [firebase/config.ts](src/lib/firebase/config.ts) switched from `getFirestore(app)` to `initializeFirestore(app, { localCache: persistentLocalCache({ tabManager: persistentMultipleTabManager(), cacheSizeBytes: CACHE_SIZE_UNLIMITED }) })`. Reads served instantly from IndexedDB cache; writes queue transparently when offline and auto-flush on reconnect. Multi-tab safe. Falls back to `getFirestore(app)` if already initialized (HMR).
- [x] **Atomic cola take via `runTransaction`** — Fixed 2026-04-21: [waitingListService.ts](src/lib/services/waitingListService.ts) `take()` rewritten. All prep reads (entry snapshot, services, taker staff skills) happen up front; the transaction only reads the entry doc, verifies `status === 'waiting'`, and atomically writes the new session (`tx.set`) + marks entry `taken` (`tx.update`). Two workers tapping Tomar simultaneously on flaky wifi → exactly one wins, the other gets `ENTRY_ALREADY_TAKEN` and sees a friendly Spanish toast ("Otro compañero tomó esta atención primero"). Also eliminates the prior N+1 per-service updates — entire session is written in one tx.set. Error handled in both [cola/page.tsx](src/app/(app)/cola/page.tsx) and [my-work/page.tsx](src/app/(app)/my-work/page.tsx).
- [x] **Connection-state pill** — Fixed 2026-04-21: New [ConnectionPill](src/components/ConnectionPill.tsx) component + [useConnectionState](src/hooks/useConnectionState.ts) hook. Three states derived from `navigator.onLine` + an in-memory pending-writes counter: green "En línea" when online and idle, amber pulsing "Sincronizando · N" when writes are queued, red "Sin conexión" when offline. Pending counter tracked via [connectionState.ts](src/lib/firebase/connectionState.ts) module and incremented inside `addDocument`/`updateDocument`/`deleteDocument`/`batchUpdate`/`take` transaction. Mounted in both the mobile top bar (compact) and desktop sidebar footer (full). Tooltip includes offline hint "Tus cambios se enviarán al volver la señal".
