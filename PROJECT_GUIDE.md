...
- [ ] **Materials should NOT be charged to the client** — Currently `totalAmount = servicePrices + materialSellPrices`. Per business rule: client pays only service price. Material cost is INTERNAL — used only for staff commission deduction (`commission = (servicePrice - materialBuyCost) * rate`). Must fix: (1) `SessionService.addServiceToSession()` and `closeSession()` — remove material sell price from totalAmount; (2) `SessionCard` summary — remove "Materiales" subtotal row from client-facing view; (3) `ReceiptModal` — remove materials section entirely, show only services + total + payment detail; (4) WhatsApp receipt text — same cleanup. This is the #1 priority fix. (4.16, 4.31, 4.35 from manual testing)
- [ ] **"Sin Cliente (Eventual)" missing in Admin/Gerente sessions page** — The walk-in sentinel option `{ value: '__walkin__', label: 'Sin Cliente (Eventual)' }` was added to my-work page only. The sessions/page.tsx "Nuevo Trabajo" modal uses a separate client list without this option. Add it as the first option in the sessions page client SearchableSelect too. (4.5, 4.6)

#### P5-HIGH — Confirmed Bugs

- [ ] **Mis Ganancias date selector broken for past dates** — `useRealtime` with Firestore `where('date', '==', selectedDate)` works for today but not for past dates. Likely a timezone issue: server stores date in UTC while Bolivia is UTC-4. Sessions created on 2026-03-29 at 20:00 local time are stored as `date: '2026-03-30'` in Firestore. Fix: use `America/La_Paz` timezone when computing `selectedDate` for both display and Firestore queries. Affects my-earnings and my-appointments too. (5B.4, 5B.5, 5C.4, 5C.5)
- [ ] **Client duplicate phone validation not working** — Creating a client with an existing phone succeeds instead of showing a duplicate error. `ClientRepository` has phone uniqueness check but it may not be firing correctly. Debug `getSalonClientByPhone()` call path. (6.6)
- [ ] **No "Cancelar cita" button in appointments page** — The appointments list has no way to cancel a confirmed or pending appointment. Need a cancel action on each appointment card (admin/manager only). (7.6)
- [ ] **Multi-location selector not switching data** — Changing salon from the sidebar selector in `/salons` does not update the active `salonId` context. All pages keep showing data from the original salon. (16.3)
- [ ] **Anular trabajo from previous day fails silently** — Cancelling a session from a previous day does not show it as cancelled in the UI and does not restore stock. Root cause likely: the `cancelSession()` call succeeds but the session's `date` field doesn't match `selectedDate`, so the real-time subscription doesn't include it in the current view. Fix: ensure cancelled sessions are shown regardless of date filter, or the Firestore query includes `cancelled` status from any date. (4.39, 4.40)
- [ ] **Anular should be admin-only, not gerente** — Currently `canCancel` is true for both admin and manager roles. User requirement: only admin can void sessions. Change `canCancel` logic in sessions/page.tsx to check `userRole === 'admin'` only. (4.39, roles matrix)
- [ ] **Cash change color not red when amount < total (second tester)** — Payment modal change calculator appears to show negative change without red color for one tester. Verify the conditional `text-red-600` class logic in the payment modal for the change amount display. (4.22)

#### P5-HIGH — UX Blocking Issues

- [ ] **Split payment UX: users don't understand to change per-person amount** — When splitting, both entries default to the full `remaining` amount, resulting in doubled totals (e.g. 60 + 30 = 90 shown as total). Fix: when "+ Agregar Pago" is pressed, auto-split the remaining amount equally, OR show the total remaining prominently and lock it so each entry's amount is validated to not exceed remaining. The goal: staff should never need to manually calculate what each person owes. (4.27)
- [ ] **Client search broken in my-work "Nuevo Trabajo" modal** — Typing in the client SearchableSelect makes the dropdown disappear instead of filtering. Likely a focus or state management bug in the SearchableSelect inside the my-work modal context. (5.9)
- [ ] **Saldo payment at full amount blocks payment modal** — If client's saldo covers the entire session amount, the payment total goes to Bs. 0.00 and the "Confirmar Pago" button is disabled (zero-amount guard). Need: if saldo discount brings total to ≤0, auto-complete the payment without requiring a payment method entry. Also clarify: saldo payment does NOT appear in daily earnings (correct behavior — the cash was collected on a previous day), but staff commission should still be earned. Document this and add a note in the daily earnings summary. (6.13)
- [ ] **Gastos edit/delete should be admin-only** — Currently both admin and manager can edit and delete expenses. Business requirement: only admin. Add role check in gastos/page.tsx to hide Edit and Delete buttons when `userRole !== 'admin'`. (13.3, 13.4, 13.5)
- [ ] **Loyalty reward redemption only from /clients page** — Users must navigate away from the payment flow to redeem points. Add a "Canjear Recompensa" button inside the payment modal that opens a reward picker and applies the discount to the session total. (6.11, 14.4)

#### P5-MEDIUM — UX Improvements

- [ ] **Date range selector for Dashboard, Gastos, Citas** — Dashboard currently selects a single date. Users want to query a range (e.g. this week, last 7 days). Add "Desde"/"Hasta" date inputs with presets: Hoy, Ayer, Esta Semana, Este Mes. Affects: dashboard/page.tsx, gastos/page.tsx (currently month-only), appointments/page.tsx. (3.5, 13.6, 7.7)
- [ ] **Staff dashboard: hide "Cierre de Caja" section + fix KPI label** — Staff can still see the "Cierre de Caja" section which is admin/manager operational info. Wrap it with `{!isStaff && ...}`. Also rename the revenue KPI card from "ingresos totales" → "Mis Ingresos del Día" to match personal framing. (3B.1, 3B.2)
- [ ] **Service without staff should appear in "Trabajos Disponibles"** — When admin adds a service to a session without assigning a staff member (`assignedStaff: []`), it should appear in the my-work "Trabajos Disponibles" section. Currently these services are invisible to staff. Fix: in my-work, unassigned services query should match `assignedStaff.length === 0` or `!assignedStaff`. (4.15)
- [ ] **No way to add/edit materials after service is already saved** — Once a service is added to a session, there is no button to add materials retroactively. The "Agregar Material" option only exists during service creation. Add a material edit icon/button on each service row in the active session card (admin/manager only) that opens the material form for that service. (4.18)
- [ ] **Decimal input uses comma, not period; elderly users struggle** — On some Android keyboards the decimal separator is comma (`,`) but the input expects period (`.`). Also, elderly users find typing `0.5` unintuitive. Fix both: (1) normalize comma→period on input change; (2) add preset quantity buttons: `¼`, `½`, `¾`, `1`, `2` as tap targets above the number input for material quantities. (5.7)
- [ ] **Material modal buttons covered by keyboard on mobile** — In the material logging modal on my-work, the Cancelar/Guardar buttons scroll out of view when the numeric keyboard opens. Fix: add `padding-bottom` equal to keyboard height using the `dvh` + scroll trick already used in other modals, or move buttons to the top of the modal. (5.8)
- [ ] **Service selector in my-work shows all services, not staff's specialties** — When creating a trabajo from my-work, the service dropdown lists all salon services. It should pre-filter to only services listed in the staff member's `specialties` array (from the staff record). Show a note if their specialties list is empty. (5.11)
- [ ] **Mis Reservas: cita card missing service name + no worker confirm button** — Appointment cards in my-appointments show client name, time, and status but not which service was requested. Add service name display. Also add a "Confirmar" button for the staff member to acknowledge the appointment (changes status from `pending` → `confirmed`). (5C.3)
- [ ] **Client "Total Gastado" shows 0 instead of actual spend** — The client list card shows "Total gastado" but it's always blank or zero. This should aggregate all completed session `totalAmount` values for that client. Likely `client.totalSpent` field is not being updated when sessions close. Fix `closeSession()` in sessionService to update `client.totalSpent += session.totalAmount`. (6.8)
- [ ] **Planilla payment not marked as paid after registering** — After clicking "Registrar Pago" in reports, the expense is created in gastos but the planilla card still shows the same amount as unpaid. It should show a "Pagado" badge and the period should reset. Add `isPaid: true` + `paidAt` tracking to the planilla record or re-query after payment. Also add payment history to Mis Ganancias so staff can see their past salary payments. (11.5)
- [ ] **Cannot delete staff with completed services** — Currently any staff member can be deleted regardless of history. Block deletion if the staff has any sessions where `services[].assignedStaff` includes their UID. Show a blocking error: "No se puede eliminar: tiene X trabajos registrados." (10.4)
- [ ] **Appointment edit + worker notification on change** — Appointments have no edit function. Add an edit button (admin/manager) that re-opens the creation form pre-filled. On save, update the record. Future: send in-app notification to assigned worker. (7, general)

#### P5-LOW — Polish & Timezone

- [ ] **Timezone: use America/La_Paz (UTC-4) for all date display and date field writes** — Sessions created late afternoon in Bolivia are stored with next-day UTC date, causing them to appear on the wrong day in reports. Fix: when computing `today` for Firestore queries and for `session.date`, use `new Date().toLocaleDateString('en-CA', { timeZone: 'America/La_Paz' })` instead of `new Date().toISOString().split('T')[0]`. Affects: sessions/page.tsx, dashboard/page.tsx, my-earnings/page.tsx, my-appointments/page.tsx, sessionService.ts. (4.30, 4.40)
