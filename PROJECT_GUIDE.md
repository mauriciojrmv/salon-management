...
- [ ] **Multi-location selector not switching data** — Changing salon from the sidebar does not update active `salonId` context. All pages keep showing data from original salon. Root cause: need to investigate how the location context propagates. (16.3)
- [ ] **Anular trabajo from previous day fails silently** — Timezone fix (above) should reduce this. But sessions from truly different UTC dates still won't appear in the real-time subscription (which queries by `date == today`). Fix: the cancel modal in sessions page should directly fetch the session by ID rather than relying on it being in the current view. Remaining work. (4.39, 4.40)
...
- [ ] **Client search broken in my-work "Nuevo Trabajo" modal** — Typing in client SearchableSelect makes dropdown disappear. Likely focus/state bug in that modal context. Needs investigation in my-work/page.tsx. (5.9)
